...
- **`vllm_config`**\
  Backend-specific engine options are passed as a nested dict in the backend config. Pass a `vllm_config` key whose value is a dict of engine option names and values.
...
- [vLLM Chat templates](https://docs.vllm.ai/en/latest/api/vllm/transformers_utils/chat_templates/) — For `request_format` behavior.
