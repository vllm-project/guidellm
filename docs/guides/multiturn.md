...
All standard benchmarking arguments apply to multiturn tasks, such as `--profile`, profile rate parameters, and `--constraint max_requests count=<n>`. Any options that operate on "requests" will treat each turn as a separate request (e.g. A dataset row with 3 turns will count as 3 requests).
...
```bash
--data synthetic_text "prompt_tokens=256,output_tokens=128,turns=3"
```
...
```bash
--data synthetic_text "prompt_tokens=256,output_tokens=128,turns=3,prefix_count=5,prefix_tokens=50"
```
...
```bash
guidellm run \
  --backend openai_http 'target=http://localhost:8000,request_format=/v1/responses,server_history=true' \
  --data synthetic_text "prompt_tokens=200,output_tokens=100,turns=3"
```
...
```bash
--data huggingface "source=dataset0.jsonl" \
--data huggingface "source=dataset1.jsonl" \
--data-preprocessor encode_media "" \
--data-preprocessor turn_pivot ""
```
...
```bash
guidellm run \
  --backend openai_http "target=http://localhost:8000,model=meta-llama/Llama-3.1-8B-Instruct,request_format=/v1/chat/completions" \
  --profile concurrent "streams=6" \
...
- `--constraint max_requests count=30`: Maximum number of requests to send (30 requests ~= 10 conversations with 3 turns each)
...
guidellm run \
  --backend openai_http "target=http://localhost:8000,model=meta-llama/Llama-3.1-8B-Instruct,request_format=/v1/chat/completions" \
  --profile constant "rate=2.0" \
  --constraint max_requests "count=100" \
  --data synthetic_text "prompt_tokens=150,output_tokens=75,turns=4,prefix_tokens=100"
```

**Key Parameters:**

- `--profile constant "rate=2.0"`: Send requests at a constant rate of 2 per second
- `--constraint max_requests "count=100"`: Maximum number of requests (100 requests ~= 25 conversations with 4 turns each)
...
> [!NOTE]\
> `turns=4` + `--constraint max_requests "count=100"` will result in 25 **or more** conversations. Follow-up turns can only be scheduled when the previous turn completes. When a turn is complete the conversation is placed at the front of the queue for the current worker.
...
- `--profile constant "rate=1.5"`: Send requests at a constant rate of 1.5 per second
- `--constraint max_duration "seconds=60"`: Run each strategy for up to 60 seconds
...
guidellm run \
  --backend openai_http "target=http://localhost:8000,model=meta-llama/Llama-3.1-8B-Instruct,request_format=/v1/chat/completions" \
  --profile concurrent "streams=10" \
  --constraint max_requests "count=200" \
  --data huggingface "source=multiturn_conversations.jsonl"
```

**Key Parameters:**

- `--profile concurrent "streams=10"`: Maintain 10 concurrent requests
- `--constraint max_requests "count=200"`: Maximum number of requests (200 requests ~= 100 conversations with 2 turns each)
...
guidellm run \
  --backend openai_http "target=http://localhost:8000,model=meta-llama/Llama-3.1-8B-Instruct,request_format=/v1/chat/completions" \
  --profile concurrent "streams=10" \
  --constraint max_requests "count=150" \
  --data synthetic_text "prefix_tokens=512,prompt_tokens=128,output_tokens=256" \
  --data synthetic_text "prompt_tokens=256,prompt_token_stdev=32,output_tokens=128" \
  --data synthetic_text "prompt_tokens=64,output_tokens=128,output_tokens_stdev=16" \
  --data-preprocessor turn_pivot ""
```

**Key Parameters:**

- `--constraint max_requests "count=150"`: Maximum number of requests (150 requests ~= 50 conversations with 3 turns each)
- `--data`: Specified separately for each dataset; can also be specified once as an array
- `--data-preprocessor turn_pivot ""`: Transpose datasets and turn columns
...
```bash
--backend openai_http 'target=http://localhost:8000,multiturn_reasoning=true'
```
...
```bash
--backend openai_http 'target=http://localhost:8000,multiturn_reasoning="Here is my thought process:{reasoning}Here is my response:"'
```

### Raw (no delimiters)

```bash
--backend openai_http 'target=http://localhost:8000,multiturn_reasoning="{reasoning}"'
```
...
For example, `--data synthetic_text "prefix_tokens=50,prompt_tokens=100,output_tokens=200,turns=5"` will have:
