...
from guidellm.backends.openai.openai_common import (
    FALLBACK_TIMEOUT,
...
class OpenAIRealtimeWebSocketBackend(Backend):
    ...
    def backend_args(cls) -> type[BackendArgs]:
        return OpenAIRealtimeWsBackendArgs
    ...
    def _resolve_validate_kwargs(
        ...
        return resolve_openai_validate_kwargs(
            validate_backend, self.target, self.api_routes
        )

    def _build_headers(
        self, existing_headers: dict[str, str] | None = None
    ) -> dict[str, str] | None:
        return build_openai_headers(self.api_key, existing_headers)
    ...
    async def resolve(  # type: ignore[override, misc]  # noqa: C901, PLR0912, PLR0915
        ...
        ws_headers = self._build_headers()
