...
class TestGoodputSearch:
    ...
    @pytest.mark.regression
    def test_conclusion_carries_the_final_probe_and_stop_reason(self):
        """
        Expose the whole search, including the last probe, through the profile
        conclusion instead of only through the per-benchmark config.
        ...
        assert [probe["streams"] for probe in conclusion["probes"]] == probed
        assert conclusion["best_passing_streams"] == search.best_passing_streams
        assert conclusion["stop_reason"] == "converged"
