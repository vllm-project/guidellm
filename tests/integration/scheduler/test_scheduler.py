...
async def test_scheduler_run_integration(
    ...
    assert len(received_responses) == constraints["max_requests"].max_num
    assert last_state.created_requests == constraints["max_requests"].max_num
    assert last_state.queued_requests == 0
    assert last_state.processing_requests == 0
    assert last_state.processed_requests == constraints["max_requests"].max_num
    assert last_state.cancelled_requests == 0
    assert (
        last_state.successful_requests + last_state.errored_requests
    ) == constraints["max_requests"].max_num
